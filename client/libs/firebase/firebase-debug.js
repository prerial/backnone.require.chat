...
fb.core.util.validation.validateFirebaseData = function (errorPrefix, data, depth, path) {
    ...
    if (goog.isString(data) && data.length > fb.core.util.validation.MAX_LEAF_SIZE_ / 3 && fb.core.util.utf8.stringToByteArray(data).length > fb.core.util.validation.MAX_LEAF_SIZE_) {
    ...
    if (goog.isObject(data)) {
        for (var key in data) {
            if (fb.util.obj.contains(data, key)) {
                if (key !== ".priority" && key !== ".value" && !fb.core.util.validation.isValidKey(key)) {
...
fb.core.util.validation.validatePriority = function (fnName, argumentNumber, priority, optional) {
    ...
    if (priority !== null && !goog.isNumber(priority) && !goog.isString(priority)) {
        throw new Error(fb.util.validation.errorPrefix_(fnName, argumentNumber, optional) + "must be a valid firebase priority " + "(null or a string.)");
    }
};
...
fb.core.util.validation.validateKey = function (fnName, argumentNumber, key, optional) {
    ...
    if (!fb.core.util.validation.isValidKey(key)) {
        throw new Error(fb.util.validation.errorPrefix_(fnName, argumentNumber, optional) + 'must be a valid firebase key (non-empty string, not containing ".", "#", "$", "/", "[", or "]").');
    }
};
fb.core.util.validation.validatePathString = function (fnName, argumentNumber, pathString, optional) {
    ...
    if (!fb.core.util.validation.isValidPathString(pathString)) {
        throw new Error(fb.util.validation.errorPrefix_(fnName, argumentNumber, optional) + "must be a non-empty string and " + 'can\'t contain ".", "#", "$", "[", or "]".');
    }
};
...
fb.core.snap.NodeFromJSON = function (json, opt_priority) {
    ...
    fb.core.util.assert(priority === null || typeof priority === "string" || typeof priority === "number");
...
fb.core.snap.buildChildSet = function (childList, childData, usePriority) {
    var cmp = usePriority ? fb.core.snap.NAME_AND_PRIORITY_COMPARATOR : fb.core.snap.NAME_COMPARATOR;
    childList.sort(cmp);
    ...
    if (root !== null) {
        return new fb.core.util.SortedMap(usePriority ? cmp : null, root)
    } else {
        return new fb.core.util.SortedMap(usePriority ? cmp : null)
    }
};
...
fb.realtime.BrowserPollConnection["isAvailable"] = function () {
    return !fb.realtime.BrowserPollConnection.forceDisallow_ && !fb.core.util.isChromeExtensionContentScript() && (fb.realtime.BrowserPollConnection.forceAllow_ || !NODE_CLIENT)
};
...
function FirebaseIFrameScriptHolder(commandCB, onMessageCB, onDisconnectCB, urlFn) {
    ...
    if (!NODE_CLIENT) {
        ...
        var script = "";
        if (this.myIFrame.src.substr(0, "javascript:".length) === "javascript:") {
...
fb.core.PersistentConnection = function (repoInfo, onDataUpdate, onConnectStatus, onAuthStatus, onServerInfoUpdate) {
    ...
    this.lastConnectionAttemptTime = null;
    this.lastConnectionEstablishedTime = null;
    this.establishConnectionTimer_ = setTimeout(goog.bind(this.establishConnection_, this), 0)
};
...
fb.core.PersistentConnection.prototype.sendRequest_ = function (action, body, onResponse) {
    ...
    if (this.connected_) {
        ...
        this.pendingSendQueue_.push(msg)
    }
    if (onResponse) {
...
fb.core.PersistentConnection.prototype.unauth = function () {
    delete this.credential_;
    this.onAuthStatus_(false);
    if (this.connected_) {
        this.sendRequest_("unauth", {})
    }
};
...
fb.core.PersistentConnection.prototype.onReady_ = function (timestamp) {
    ...
    this.lastConnectionEstablishedTime = (new Date).getTime();
    this.handleTimestamp_(timestamp);
    for (var messNum = 0; messNum < this.pendingSendQueue_.length; messNum++) {
        this.realtime_.sendRequest(this.pendingSendQueue_[messNum])
    }
    this.pendingSendQueue_ = [];
    this.restoreState_();
...
fb.core.PersistentConnection.prototype.onRealtimeDisconnect_ = function () {
    ...
    var reestablishFn = goog.bind(function () {
    ...
    if (!this.shouldReconnect_) {
        ...
        this.resume_ = function () {
            setTimeout(reestablishFn, 0)
        }
    } else {
        if (this.lastConnectionEstablishedTime) {
            var timeSinceLastConnectSucceeded = (new Date).getTime() - this.lastConnectionEstablishedTime;
            if (timeSinceLastConnectSucceeded > RECONNECT_DELAY_RESET_TIMEOUT) {
                this.reconnectDelay_ = RECONNECT_MIN_DELAY
            }
            this.lastConnectionEstablishedTime = null
        }
        var timeSinceLastConnectAttempt = (new Date).getTime() - this.lastConnectionAttemptTime;
        var reconnectDelay = Math.max(0, this.reconnectDelay_ - timeSinceLastConnectAttempt);
        reconnectDelay = Math.random() * reconnectDelay;
        this.log_("Trying to reconnect in " + reconnectDelay + "ms");
        setTimeout(reestablishFn, reconnectDelay);
...
fb.core.PersistentConnection.prototype.establishConnection_ = function () {
    if (this.shouldReconnect_) {
        ...
        this.lastConnectionAttemptTime = (new Date).getTime();
        this.lastConnectionEstablishedTime = null;
    ...
    if (goog.isDef(query.startPriority)) {
        if (goog.isDef(query.startName) && query.startName != null) {
            constraints.push(function (name, priority) {
                ...
                return priorityDiff > 0 || priorityDiff === 0 && name >= query.startName
            })
        } else {
            constraints.push(function (name, priority) {
    ...
    if (goog.isDef(query.endPriority)) {
        if (goog.isDef(query.endName)) {
            constraints.push(function (name, priority) {
                ...
                return priorityDiff < 0 || priorityDiff === 0 && name <= query.endName
            })
        } else {
            constraints.push(function (name, priority) {
...
fb.core.view.QueryView.prototype.getChildRelevance = function (path, newNode, serverData) {
    ...
    this.processChanges_(newNode, allChanges);
    if (!this.snapshotNode_.isLeafNode()) {
        this.snapshotNode_.forEachChild(function (childName) {
            childRelevance[childName] = true
        })
...
fb.core.ViewManager.prototype.raiseEventsForChange = function (changePath, completeQueryPaths) {
    ...
    var markComplete = function (path) {
        for (var i = 0; i < completeQueryPaths.length; ++i) {
            var parentPath = completeQueryPaths[i];
    ...
    var onDiff = function (path, snapNode, changes) {
        if (changePath.contains(path)) {
            var markQueriesComplete = markComplete(path);
    ...
    if (this.connection_) {
        ...
        this.connection_.listen(queryMap, function (status) {
            if (status !== "ok") {
                ...
                goog.array.forEach(qids, function (qid) {
                    var view = queryMap.get(qid);
                    view && view.raiseQueuedEvents()
                })
            }
        });
        return goog.bind(this.connection_.unlisten, this.connection_, queryMap.path(), queryMap.toString(), queryMap.queries())
    } else {
...
fb.core.ViewManager.prototype.getChildRelevance = function (path, queryMap, newNode, mergedData) {
    ...
    var updateChildRelevance = function (newRelevances) {
        goog.object.forEach(newRelevances, function (relevant, childName) {
            if (relevant) {
                childRelevance[childName] = true
            } else {
                childRelevance[childName] = fb.util.obj.get(childRelevance, childName) || false
            }
        })
    };
    queryMap.each(function (queryId, view) {
    ...
    if (!newNode.isLeafNode()) {
        newNode.forEachChild(function (childName) {
            if (!fb.util.obj.contains(childRelevance, childName)) {
                childRelevance[childName] = false
            }
        })
    }
    return childRelevance
};
fb.core.ViewManager.prototype.isInParentView = function (path, newNode, mergedData) {
    var previousTree = this.viewsTree_.subTree(path);
    var subTree = previousTree.parent();
    var found = false;
    while (!found && subTree !== null) {
        ...
        if (queryMap !== null) {
            if (queryMap.hasDefaultQuery()) {
                ...
                var pathSegment = previousTree.name();
                if (fb.util.obj.get(childRelevance, pathSegment)) {
                    found = true
                }
            }
        }
...
fb.core.ViewManager.prototype.pruneNonDefaultQuery = function (queryMap, viewNode, newNode, mergedData, mergedChildren) {
    var path = queryMap.path();
    if (mergedChildren !== null) {
        var updates = [];
        if (!newNode.isLeafNode()) {
            newNode.forEachChild(function (childName, childNode) {
                updates.push({ path: path.child(childName), node: childNode });
                delete mergedChildren[childName]
            })
        }
        goog.object.forEach(mergedChildren, function (data, childName) {
            updates.push({ path: path.child(childName), node: fb.core.snap.EMPTY_NODE })
        });
        return updates
    } else {
        var childRelevance = this.getChildRelevance(path, queryMap, newNode, mergedData);
        ...
        var childListeners = [];
        var self = this;
        goog.object.forEach(childRelevance, function (relevant, childName) {
...
fb.core.ViewManager.prototype.pruneUpdateNode = function (path, newNode, mergedData, mergedChildren) {
    if (this.isInParentView(path, newNode, mergedData)) {
        ...
        if (queryMap !== null) {
            if (queryMap.hasDefaultQuery()) {
                ...
                return this.pruneNonDefaultQuery(queryMap, updateRoot, newNode, mergedData, mergedChildren)
            }
        } else {
            return this.pruneObjectToListeners_(newNode, updateRoot, mergedData)
        }
    }
};
fb.core.ViewManager.prototype.pruneObjectToListeners_ = function (node, subTree, mergedData) {
    ...
    if (queryMap !== null) {
        if (queryMap.hasDefaultQuery()) {
            ...
            return this.pruneNonDefaultQuery(queryMap, subTree, node, mergedData, null)
        }
    } else {
        var snapshots = [];
        var self = this;
...
fb.core.Repo = function (repoInfo) {
    this.repoInfo_ = repoInfo;
    this.stats_ = fb.core.stats.StatsManager.getCollection(repoInfo);
    this.connection_ = new fb.core.PersistentConnection(this.repoInfo_, goog.bind(this.onDataUpdate_, this), goog.bind(this.onConnectStatus_, this), goog.bind(this.onAuthStatus_, this), goog.bind(this.onServerInfoUpdate_, this));
    ...
    this.updateInfo_("serverTimeOffset", null);
    ...
    if (pathString.length >= 9 && pathString.lastIndexOf(".priority") === pathString.length - 9) {
        ...
        if (isMerge) {
            ...
            for (var childName in data) {
                var childPath = new fb.core.util.Path(childName);
                newNode = newNode.updateChild(childPath, fb.core.snap.NodeFromJSON(data[childName]));
...
fb.core.Repo.prototype.unauth = function () {
    this.connection_.unauth()
};
...
fb.core.Repo.prototype.update = function (path, childrenToMerge, onComplete) {
    ...
    var prunedNodes = this.viewManager_.pruneUpdateNode(path, updatedNode, this.data_.mergedData, childrenToMerge);
    var setIds = this.data_.set(path, prunedNodes);
    var self = this;
...
fb.core.Repo.prototype.runOnDisconnectEvents_ = function () {
    ...
    var self = this;
    var setIds = [];
    this.onDisconnect_.forEachTree(new fb.core.util.Path(""), function (path, tree) {
        var prunedNodes = self.viewManager_.pruneUpdateNode(path, tree, self.data_.mergedData, null);
